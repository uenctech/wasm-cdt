...
static constexpr uint64_t INDEX_SCOPE_RESEVED = 0;
...
TABLE  TABLE_IN_CONTRACT account {
   ...
   uint64_t get_symbol_code()const { return balance; }
   std::string    get_owner()const{ return owner; }
};

typedef wasm::index_table< "accounts"_n, account, uint64_t,
                           indexed_by<"by_owner"_n, const_mem_fun<account, std::string, &account::get_owner>>,
