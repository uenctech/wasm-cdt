...
namespace wasm {   
   struct dsmalloc {
      ...
      char* operator()(size_t sz, uint8_t align_amt=8) {
         ...
         size_t pages_to_alloc = sz >> 16;   // 分配的内存占的页数  64k每页
         next_page += pages_to_alloc;
         if ((next_page << 16) <= (size_t)last_ptr) {
            next_page++;
            pages_to_alloc++;
         }         
         wasm::check(GROW_MEMORY(pages_to_alloc) != -1, "failed to allocate pages");  
         return ret;
      }
      //主要维护着1.last_ptr指针涨到了哪里 2.next_page目前为止一共分配多少页
      char*  heap;
      char*  last_ptr;
      size_t offset;
      size_t next_page;
...
void* malloc(size_t size) {
   void* ret = wasm::_dsmalloc(size);
   return ret;
}

void* memset(void*,int,size_t);
void* calloc(size_t count, size_t size) {
   if (void* ptr = wasm::_dsmalloc(count*size)) {
